...
.. note:: Network converters are not in :mod:`biryani1.baseconv`, because they use non-standard network libraries.
"""
...
u'An error occured when trying to connect to the web server: <urlopen error [Errno 2] No such file or directory>')
    >>> test_http_url()(u'')
    Traceback (most recent call last):
    ValueError:
    >>> pipe(
