...
...
>>> french_translation = gettext.translation('biryani1',
...     os.path.join(pkg_resources.get_distribution('biryani1').location, 'biryani1', 'i18n'),languages = ['fr'])
...
...
.. note:: If you get an ``IOError: [Errno 2] No translation file found for domain: 'biryani1'``, your *Biryani*
    ...
    ``gettext.translation``, like::
        ...
        french_translation = gettext.translation('biryani1', localedir = 'path/to/biryani1/i18n', languages = ['fr'])

    or, if the content of ``biryani1/i18n`` directory has been copied in ``/usr/local/share/local``::

        french_translation = gettext.translation('biryani1', '/usr/local/share/locale', languages = ['fr'])
...
...             self._translations_cache[languages] = translation = gettext.translation('biryani1',
...
For example, if the application using *Biryani* is named **myapp** and stores its localized messages in ``myapp/i18n`` you could chain its translations with those of *Biryani* using some code like::
    ...
    languages = ['fr']
    biryani_translation = gettext.translation('biryani1',
        os.path.join(pkg_resources.get_distribution('biryani1').location, 'biryani1', 'i18n'), fallback = True,
        languages = languages)
